...
from data.dataset import FMRIGraphDataset
...
def _cache_file(
    ...
    key = (
        f"{os.path.basename(raw_path)}_{train_ratio}_{val_ratio}_{test_ratio}_"
...
def load_data(
    ...
    """Load, split, optionally augment, and wrap fMRI graph data into PyG DataLoaders.

    Parameters
    ----------
    path:
        Path to a ``.pt`` file containing a list of ``torch_geometric.data.Data`` objects.
    batch_size:
        Mini-batch size for all loaders.
    workers:
        Number of DataLoader worker processes.
    train_ratio, val_ratio, test_ratio:
        Proportions for the three splits; must sum to 1.
    augment_strategy:
        One of ``"upsample"``, ``"interpolate"``, ``"geodesic"``, or ``None``.
    augment_proportion:
        Target proportion of the largest class used by the augmentation strategy.
        Required when ``augment_strategy`` is not ``None``.
    cache_root:
        Directory for caching the split/augmented lists to avoid recomputation.

    Returns
    -------
    train_loader, val_loader, test_loader, results_dict
        ``results_dict`` contains ``"class_weights"`` (inverse-frequency weights for
        the training split).
    """
    ...
    cache_path = _cache_file(
        path, cache_root, train_ratio, val_ratio, test_ratio,
    ...
    train_dataset = FMRIGraphDataset(train_data)
    val_dataset   = FMRIGraphDataset(val_data)
    test_dataset  = FMRIGraphDataset(test_data)
