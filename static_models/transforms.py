...
class BatchNodeFeatureTransform(BaseTransform):
    """
    Take the stacked `.c` field on a PyG Batch (shape = [B*N, N]) and
    assign it as the node‐feature tensor `.x`.

    Leaves `batch.metadata` alone and does NOT delete `.c` in case
    you need it for downstream transforms (like BatchEdgeListTransform).
    """
    def __call__(self, batch):
        if not hasattr(batch, "c"):
            raise AttributeError("Batch must have attribute `c` (shape [B*N, N])")
        # Directly use the existing stacked correlation rows as node features:
        batch.x = batch.c
        return batch
...
class BatchEdgeListTransform(BaseTransform):
    ...
    matrices live in `batch.c` (shape = (B*N, N)).
    ...
    def __call__(self, batch):
        if not hasattr(batch, "c"):
            raise AttributeError(
                "Batch must contain attribute `batch.c` of shape (B*N, N)."
            )

        # ────────── reshape to (B, N, N) ──────────
        total_rows, N = batch.c.shape                  
        if total_rows % N != 0:
            raise ValueError("batch.c rows not divisible by N; graphs have unequal sizes?")
        B = total_rows // N
        C = batch.c.view(B, N, N)                       # (B, N, N)
        dev = C.device
        ...
        if self.tsh is None:
            k = int(self.top * vals.size(1))                     # edges to keep per graph
            if k == 0:
