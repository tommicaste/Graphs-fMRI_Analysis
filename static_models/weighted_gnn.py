...
from static_models.transforms import BatchEdgeListTransform, BatchFeatureTransform
...
class WeightedGNN(pl.LightningModule):
    def __init__(
        self,
        input_dim: int,
        hidden_channels: int,
        ...
        for i, (conv, norm) in enumerate(zip(self.convs, self.norms)):
            ...
            if edge_weight is not None:
                ...
                if "edge_weight" in sig.parameters:
                    edge_kwargs["edge_weight"] = edge_weight
                elif "edge_attr" in sig.parameters:
                    edge_kwargs["edge_attr"] = edge_weight

            x = conv(x, edge_index, **edge_kwargs)
            x = norm(x).relu()
    ...
    def training_step(self, batch, _):
        ...
        self.log(
            "train_loss", loss, on_epoch=True, prog_bar=False, batch_size=batch.y.size(0), sync_dist=True
        )
        return loss
    ...
    def validation_step(self, batch, _):
        ...
        self.log(
            "val_loss", loss, on_epoch=True, prog_bar=False, batch_size=batch.y.size(0), sync_dist=True
        )
    ...
    def configure_optimizers(self):
        ...
        wd = float(self.hparams.get("wd", 1e-3))
