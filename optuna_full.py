...
# ───────────────────── 0. SETUP ─────────────────────
ROOT = Path("non_overlapping_fully_connected_optuna")
...
# ——— CHANGED: removed BOTH pooling_fn and edge_dropout here
FIXED_MODEL = dict(
    input_dim=347,
    dropout=0.5,
    num_classes=4,
...
def run_stage(name, sampler_fn, n_trials, pruner):
    ...
    if finished < n_trials:
        study.optimize(make_objective(sampler_fn),
                       n_trials=n_trials - finished, gc_after_trial=True)
    return study
